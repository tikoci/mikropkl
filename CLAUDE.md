...
feature gap; see the FEAT_SSBS section below.
...
Grounded on the reporter's M4 in [#11](https://github.com/tikoci/mikropkl/issues/11) —
not reproducible locally (this is an Intel Mac; aarch64 HVF needs Apple Silicon):
...
if [ "$(sysctl -n hw.optional.arm.FEAT_SSBS 2>/dev/null || echo 1)" = "0" ]; then
  ...
  SSBS_TCG=1           # drives the one-line note in the launch banner
else
  ACCEL="hvf"
fi
```
...
No QEMU release fixes this yet: the HVF SSBS shim is an
[unmerged upstream RFC](https://lists.libreplanet.org/archive/html/qemu-arm/2025-10/msg00421.html)
(Oct 2025, private Apple API, aimed at macOS guests) — not in 11.0.2, the 11.1 rc, or
`--HEAD`.  Because the host sysctl will still read `0` when a fixed QEMU ships, restoring
HVF then needs a **QEMU-version floor added at this fallback site**; deliberately deferred
until there is a release to key on.  `quickchr` applies the identical probe in
`src/lib/platform.ts` ([quickchr#98](https://github.com/tikoci/quickchr/pull/98)) — keep
the two in sync.
