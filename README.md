...
CHR Releases from 7.23 onward each QEMU-backend `.utm` bundle includes a `qemu.sh` launcher and `qemu.cfg` configuration file for running the VM directly under QEMU — without UTM or libvirt.  These are useful for testing, CI, troubleshooting UTM issues, and running CHR on Linux with pre-configured settings.  Currently, used to validate CHR images in GitHub Actions, but could be used for other purposes beyond UTM, like use on Linux.

> `*.apple.*.utm` bundles use Apple Virtualization, not QEMU. So QEMU launch scripts are not included.  Apple Virtualization support sparse bundles, which may be used in future, and be unsupported by `qemu`.  Use the match `*.qemu.*.utm` bundles for QEMU launch scripts
...
### `libvirtOutput` and `qemuOutput` controls

By default, QEMU scripts (`qemu.cfg` + `qemu.sh`) are generated for every QEMU-backend machine.  Libvirt XML is not generated by default.  Control this with environment variables during `make`:

The `pkl` code has experimental support to generate a `libvirt.xml`, but largely untested, so disabled by default. `qemu.sh`/`qemu.cfg` are by default generated into bundles, but not used by UTM.

In `pkl` Templates, `libvirtOutput` and `qemuOutput` booleans can be set to control output on the non-UTM formats.  Currently `config.plist` for UTM is always generated, and cannot be

```sh
# Disable QEMU scripts
QEMU_OUTPUT=false make CHR_VERSION=7.22

# Enable libvirt XML alongside QEMU scripts
